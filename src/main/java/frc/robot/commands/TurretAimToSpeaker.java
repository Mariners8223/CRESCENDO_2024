...
public class TurretAimToSpeaker extends InstantCommand {
  ...
  public void initialize() {
    YaxisOfTargetInSpeaker = Constants.Arm.SpeakerBottomLocationY
    + Constants.Arm.SpeakerLength - Constants.Arm.SpeakerIsCenterRatio * (RobotContainer.driveBase.getPose().getTranslation().getY()
     - Constants.Arm.SpeakerIsCenterRatioBottomLocation);//this is the y axis of the wanted point
    ...
    + RobotContainer.arm.getShooterSub().getShooterVelocity()*Math.cos(RobotContainer.arm.getShooterSub().getTrueGamePieceVelocityAngle_RobotRelative_ArialView())));//not very accurate
